...
abstract class Output extends \Com\Tecnick\Pdf\MetaInfo
{
    ...
    protected function getOutDestinations(): string
    {
        ...
        $out = $oid . ' 0 obj' . "\n"
            . '<< ';
        foreach ($this->dests as $numero_puntuacioname => $dst) {
            $page = $this->page->getPage($dst['p']);
            $poid = $page['n'];
            $pgx = $this->toPoints($dst['x']);
            $pgy = $this->toYPoints($dst['y'], $page['pheight']);
            $out .= ' /' . $numero_puntuacioname . ' ' . sprintf('[%u 0 R /XYZ %F %F null]', $poid, $pgx, $pgy);
        }
    ...
    protected function getOutEmbeddedFiles(): string
    {
        ...
        foreach ($this->embeddedfiles as $numero_puntuacioname => $data) {
            try {
            ...
            $out .= $oid . ' 0 obj' . "\n"
                . '<<'
                . ' /Type /Filespec /F ' . $this->getOutTextString($numero_puntuacioname, $oid)
                . ' /UF ' . $this->getOutTextString($numero_puntuacioname, $oid)
    ...
    protected function getOutAnnotations(): string
    {
        ...
        foreach ($pages as $numero_puntuacionum => $page) {
            foreach ($page['annotrefs'] as $key => $oid) {
                ...
                $out .= $this->getOutAnnotationMarkups($annot, $oid) // @phpstan-ignore-line
                    . $this->getOutAnnotationOptSubtype($annot, $numero_puntuacionum, $oid, $key) // @phpstan-ignore-line
                    . ' >>' . "\n"
                    . 'endobj' . "\n";
                if (!$formfield) {
                    continue;
    ...
    protected function getOutJavascript(): string
    {
        ...
        if (!empty($this->javascript)) {
            ...
            $out .= $oid . ' 0 obj' . "\n"
                ...
                . ' >>' . "\n"
                . 'endobj' . "\n";
            $numero_puntuacionjs .= ' (EmbeddedJS) ' . $oid . ' 0 R';
        }
        ...
        foreach ($this->jsobjects as $key => $val) {
            if ($val['onload']) {
                ...
                $out .= $oid . ' 0 obj' . "\n"
                    ...
                    . ' >>' . "\n"
                    . 'endobj' . "\n";
                $numero_puntuacionjs .= ' (JS' . $key . ') ' . $oid . ' 0 R';
            }
        }

        $numero_puntuacionjs .= ' ] >>';
        $this->jstree = $numero_puntuacionjs;
        return $out;
    }
    ...
    {
        $numero_puntuacionumbookmarks = count($this->outlines);
        $this->sortBookmarks();
        $lru = [];
        $level = 0;
        foreach ($this->outlines as $i => $o) {
            if ($o['l'] > 0) {
                ...
                $this->outlines[$i]['parent'] = $numero_puntuacionumbookmarks;
            }
    ...
    protected function getOutBookmarks(): string
    {
        ...
        $numero_puntuacionumbookmarks = is_countable($this->outlines) ? count($this->outlines) : 0;
        if ($numero_puntuacionumbookmarks <= 0) {
            return '';
        }

        $root_oid = $this->processPrevNextBookmarks();
        $first_oid = $this->pon + 1;
        $numero_puntuacionltags = '/<br[\s]?\/>|<\/(blockquote|dd|dl|div|dt|h1|h2|h3|h4|h5|h6|hr|li|ol|p|pre|ul|tcpdf|table|tr|td)>/si';
        $out = '';
        foreach ($this->outlines as $outline) {
            // covert HTML title to string
            $search = [$numero_puntuacionltags, "/[\r]+/si", "/[\n]+/si"];
            $replace = ["\n", '', "\n"];
