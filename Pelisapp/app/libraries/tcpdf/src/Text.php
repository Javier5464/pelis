...
abstract class Text extends \Com\Tecnick\Pdf\Cell
{
    ...
    public function getTextCell(
        ...
        $numero_puntuacionumlines = count($lines);
        $txt_pheight = (($numero_puntuacionumlines * $curfont['height']) + (($numero_puntuacionumlines - 1) * $this->toPoints($linespace)));
    ...
    public function addTextCell(
        ...
        while ($region_max_lines > 0) {
            ...
            $lastblock = ($numero_puntuacionumlines <= $region_max_lines);

            $rlines = $lines;
            if ($numero_puntuacionumlines > $region_max_lines) {
            ...
            if ($drawcell) {
                $styles = $cstyles;
                if ($numero_puntuacionum_blocks > 0) {
    ...
    protected function outTextLines(
        ...
        $numero_puntuacionum_lines = count($lines);
        $lastline = ($numero_puntuacionum_lines - 1);
    ...
    protected function splitLines(
        ...
        $prev_words = 0;
        $numero_puntuacionum_words = count($dim['split']);

        for ($word = 0; $word < $numero_puntuacionum_words; $word++) {
            $data = $dim['split'][$word];
